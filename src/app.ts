...
export class FlowFieldCaseStudyApp {
  ...
  private async runComparison(): Promise<void> {
    ...
    this.elements.comparisonStatus.textContent = 'Running actual searches...';
    ...
    this.comparison = {
      flowDuration: flowStats.duration,
      flowNodes: flowStats.expandedNodes,
      aStarDuration: performance.now() - startedAt,
      aStarNodes,
      routes,
    };
  ...
  private renderCanvas(): void {
    ...
    for (const agent of this.crowd.agents) {
      if (agent.reached) {
        context.fillStyle = midnight ? 'rgba(240, 143, 97, 0.7)' : 'rgba(184, 75, 33, 0.7)';
      } else if (agent.tint < 0.34) {
        context.fillStyle = midnight ? '#73d1c5' : '#147f85';
      } else if (agent.tint < 0.67) {
        context.fillStyle = midnight ? '#91b8f3' : '#456ca8';
      } else {
        context.fillStyle = midnight ? '#9be0a8' : '#3f8a58';
      }
      context.beginPath();
  ...
  private updateComparisonTelemetry(): void {
    ...
    const ratio = this.comparison.flowDuration > 0
      ...
      : 0;
    this.elements.comparisonStatus.textContent = 'Measured on this device with the current scenario.';
    ...
    this.elements.comparisonRoutes.textContent = `${this.comparison.routes.toLocaleString()} actual A* routes`;
  }
  ...
  private updateContextLabel(): void {
    ...
    const terrain = this.map.costs[index] === TERRAIN_WALL
      ...
      : this.map.costs[index] === TERRAIN_ROUGH
        ? 'rough'
        : 'normal';
    this.elements.contextLabel.textContent = `Cell ${this.hoverColumn},${this.hoverRow} · ${terrain}`;
  }
      ...
      <main class="shell">
        ...
        <section class="workspace">
          ...
          <aside class="sidebar" aria-label="Navigation controls and telemetry">
            ...
            <section class="panel">
              <div class="panel__header">
                <div><div class="panel__kicker">Inspect</div><h3>Debug layers</h3></div>
              </div>
              <div class="toggle-stack">
                <label class="toggle"><span><b>Flow vectors</b><small>Direction sampled by every agent</small></span><span class="switch"><input id="show-vectors" type="checkbox" checked /><i></i></span></label>
                <label class="toggle"><span><b>Integration heatmap</b><small>Accumulated cost to the destination</small></span><span class="switch"><input id="show-heatmap" type="checkbox" checked /><i></i></span></label>
                <label class="toggle"><span><b>A* route sample</b><small>Draw the first 28 private paths</small></span><span class="switch"><input id="show-routes" type="checkbox" checked /><i></i></span></label>
                <label class="toggle"><span><b>Pause movement</b><small>Planning controls remain available</small></span><span class="switch"><input id="pause-sim" type="checkbox" /><i></i></span></label>
              </div>
            </section>
        ...
        <section class="comparison">
          ...
          <div class="comparison__results">
            <article class="result-card result-card--flow">
              <div><span>Shared flow field</span><b>1 reusable field</b></div>
              <strong id="comparison-flow-time">—</strong>
              <small id="comparison-flow-nodes">—</small>
            </article>
            <div class="ratio-card"><span>A* / flow build cost</span><strong id="comparison-ratio">—</strong></div>
        ...
        <dialog class="dialog" id="study-dialog">
          ...
          <div class="dialog__body">
            ...
            <ul>
              ...
              <li>Local avoidance is intentionally excluded so it does not distort the global-planning comparison.</li>
